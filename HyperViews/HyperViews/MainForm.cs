...
namespace PTViewClient
{
    public partial class MainForm : Form
    {
        ...
        {
            IntPtr[] processes = (IntPtr[]) Hvlib.GetProcessesList(Hvlib.VmHandle);

            ProcessesComboBox.Items.Add("Hyper-V Cr3");
            ProcessesComboBox.Items.Add("Host OS Cr3");

            foreach (var a in processes)
            {
                ProcessesComboBox.Items.Add(a);
            }

            //ProcessesComboBox.Items.AddRange(processes);
        }

        private void ProcessesComboBox_SelectedValueChanged(object sender, EventArgs e)
        {
            string sProcId = ProcessesComboBox.SelectedItem.ToString();
            UInt64 procId = 0;

            if (sProcId == "Hyper-V Cr3")
                procId = 0xFFFFFFFF;
            else if (sProcId == "Host OS Cr3")
                procId = 0xFFFFFFFE;
            else
                procId = UInt64.Parse(sProcId);

            PTViewPanel.Visible = true;
            try
            {
                SelectedProcessDirbase = Hvlib.GetCr3((UInt64)Hvlib.VmHandle, procId);
                DirbaseLbl.Text = $"Dirbase (cr3): 0x{SelectedProcessDirbase:X8}";
                SelectedProcessDirbase = (SelectedProcessDirbase >> 12) & 0xFFFFFFFFF;
        ...
        private void cb_VmList_DropDown(object sender, EventArgs e)
        {
            ComboBox cb_box = sender as ComboBox;
            ...
            List<VmListBox> ListObj = Hvlib.EnumPartitions(ref cfg);

            cb_box.DisplayMember = "VMName";
            cb_box.ValueMember = "VmHandle";

            cb_box.DataSource = new BindingSource(ListObj, null);
        }

        private void cb_VmList_SelectedValueChanged(object sender, EventArgs e)
        {
            //ComboBox cb_box = sender as ComboBox;
            //hvtool.Hvlib.SelectVmFromCheckBox(cb_box);
        }

        private void b_SelectVm_Click(object sender, EventArgs e)
        {
            ComboBox cb_box = sender as ComboBox;
            VmListBox sl1 = cb_box.SelectedItem as VmListBox;
